...
struct TrainingController: RouteCollection {
    ...
    func trainModel(req: Request) throws -> EventLoopFuture<String> {
        ...
        return try [req1, req2, req3, req4].computeAllModels(req: req, tags: [tag1, tag2, tag3], pathsToModels: "models/", pathsToLMs: ["trained/"], testSplit: 0.25, devSplit: 0.25, epochs: 1).map { (_) -> (String) in
            print(Trainer.infer(for: [req1, req2, req3, req4], for: ["NER", "GEO"]))
